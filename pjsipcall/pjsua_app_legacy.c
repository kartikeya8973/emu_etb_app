...
//array for sip extension on which call lands
char sip_id[128];


extern info_t info_active;

extern int call_next_no;
 ...
 * Input URL.
 */
extern void ui_input_url(const char *title, char *buf, pj_size_t len,
		input_result *result)
{
...
static void ui_answer_call()
{
	...
	char buf[128] = "200"; // Hardcoding answer code for automatic call pick-up by ETB
	pjsua_msg_data msg_data;
...
extern void ui_add_account(pjsua_transport_config *rtp_cfg)
{
	...
	sprintf(id,"sip:%s@%s", info_active.e_info.sip_user_1, info_active.e_info.svr_ip_1);
	printf("id: %s \n",id);
	sprintf(registrar,"sip:%s", info_active.e_info.svr_ip_1);
	printf("registrar: %s \n",registrar);
	sprintf(realm,"asterisk");
	printf("realm: %s \n",realm);
	sprintf(uname,"%s", info_active.e_info.sip_user_1);
	printf("uname: %s \n",uname);
	sprintf(passwd,"%s", info_active.e_info.sip_pass_1);
	printf("passwd: %s \n",passwd);
...
static void ui_delete_account()
{
	...
	//ETB is registered as the account #2
	int i = 2;
...
//******************************************************************************
pthread_mutex_t mutex = PTHREAD_MUTEX_INITIALIZER; // pthread mutex declaration and Inititlization
pthread_cond_t cond = PTHREAD_COND_INITIALIZER;	// pthread condition variable declaration and Initialization
...
char menuin[80];
/* This function is called by pjsua_app_run function in pjsua_app.c*/
void legacy_etb_main(){
...
//		//To pick-up incoming call automatically
		ui_answer_call();

		//URL of the destination sipphone (used by ui_input_url function )
		sprintf(sip_id,"sip:%s@%s",info_active.e_info.emerg_ext_s1_1,info_active.e_info.svr_ip_1);

		//To hang-up incoming call automatically
		//			pjsua_call_hangup_all();

		//code runs only after button press by user

		if(call_flag == 1){

			//Disabling the user button
			button_flag = 0;

//			if(count > 0){
//				//Re-registering the sip account
//				ui_add_account(&app_config.rtp_cfg);
//			}

			sleep(1);

			//Making a call to the guard extension 100
			ui_make_new_call();

			//Waiting for call termination
			pthread_cond_wait(&cond, &mutex);

			//			//adding the sip extension sip:101@192.168.1.22
			//			sprintf(sip_id,"sip:%s@%s",info_active.e_info.emerg_ext_s1_2,info_active.e_info.svr_ip_1);
			//
			//			//Making a call to the driver extension 103
			//			ui_make_new_call();
			//
			//			//Waiting for call termination
			//			pthread_cond_wait(&cond, &mutex);

			//			//Reseting the calling next number flag
			//			call_next_no = 0;

			//Setting calling flag to 0.Then the code waits for button press which \
			will set this flag to 1 and enter if(button_flag == 1) condition
			call_flag = 0;

			//Deleting SIP account after call termination
//			ui_delete_account();
			...
			//Enabling the user button after call termination
			button_flag = 1;
		}
	}
}
