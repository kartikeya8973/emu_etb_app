...
/*--------------------------------------------*/
// For TCP client that connects to HMI
#include<stdio.h>
#include<stdlib.h>
#include<string.h>
#include<unistd.h>
...
/* user defined*/
//For calling next sip address in case the first in not answered
int call_next_no = 0;
...
//For LED
//pthread_t blinkgreenLed;
extern pthread_t blinkgreenLed;
static pthread_t blinkblueLed;


//For last call status
int lastCall_status;
...
static void on_call_state(pjsua_call_id call_id, pjsip_event *e)
{
	...
	if (call_info.state == PJSIP_INV_STATE_CALLING)
	{
		...
		//Start the dial-tone for outgoing call
		ringback_start(1);
	}
	/*--------------------------------------------*/
	...
	if (call_info.state == PJSIP_INV_STATE_DISCONNECTED) {
		...
		//call button flag(enabling button after call is disconnected)
		//   	button_flag = 1;

		/* Stop all ringback for this call */
		ring_stop(call_id);

		//Stop the dial-tone for outgoing call (when call disconnects automatically after 180s)
		ring_stop(1);
		...
		//For calling the driver if guard does not pick the call

		/*
		 * User defined: Call_manager_code
		 * Different Call Status:
		 * 480: (Temporarily unavailable)
		 * 403: If Invite (Hangup)
		 * 486: If Invite not attending Call(Busy Here)
		 * 404: (User not Found)
		 * 487: (Request Terminated)
		 * 200: (Normal call clearing)
		 * 408: (Request Timeout)
		 */

		lastCall_status = call_info.last_status;

		pj_str_t state;
		state = call_info.last_status_text;
		...
		/* Dump media state upon disconnected */ //USE THIS FOR CALL_HISTORY
		if (1) {
			PJ_LOG(5,(THIS_FILE,
		...
		pj_bool_t *has_error)
{
	...
	if(status_name[ci->media[mi].status] == "Active")
	{
		printf("\n*\n*\n*\n*\n* ON-GOING CALL f*\n*\n*\n*\n*\n");
		pthread_cancel(blinkgreenLed);
		greenLed();

		//Stop the dial-tone for outgoing call (when call is picked at destination)
		ring_stop(1);
	}
	/*--------------------------------------------*/
...
static pj_status_t on_snd_dev_operation(int operation)
{
	...
	/*--------------------------------------------*/
	// This is also a way for indicating ringing.
	// Call thread for blinking green LED when sound device turns ON
	//	CASE 2
//	if(operation == 1) //when call connects sound device turns ON
//	{
//		printf("\n*\n*\n*\n*\n*RINGING*\n*\n*\n*\n*\n");
//		pthread_create(&blinkgreenLed, NULL, (void *)greenblink, NULL);
//	}
	...
	if(operation == 0) //when call disconnects sound device turns OFF
	{
		printf("\n*\n*\n*\n*\n* CALL TERMINATED *\n*\n*\n*\n*\n");
		redTerm();
		pthread_cancel(blinkgreenLed);

		// Create TCP client to connect to HMI.
		...
		//addr.sin_port = port;		       // port we have defined
		...
		bzero(buffer, 2048);
		strcpy(buffer,"EN:"); // Sending "EN:" string would close the CCTV view for respective ETB on the HMI.
		printf("Client: %s\n", buffer);
...
pj_status_t pjsua_app_run(pj_bool_t wait_telnet_cli)
{
	...
	status = pjsua_start();
	/*--------------------------------------------*/
	//CASE 1 - IDLE / AVAILABLE indication
	if(status == PJ_SUCCESS){

		// Pinging Network , if successful then enabling
		if (system("ping -c 2 8.8.8.8 ; echo $?") == 0)
		{
			printf("\n*\n*\n*\n*\n* READY *\n*\n*\n*\n*\n");
			redLed();
		}
	...
	if (app_config.use_cli)
		...
		//	This function is used for making a call
		legacy_etb_main();
